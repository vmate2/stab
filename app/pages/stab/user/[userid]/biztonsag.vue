<template>
  <main-frame id="frameProfile">
    <div class="frameProfile" v-if="currentUser">
      ...
      <form @submit.prevent="setNewData" name="userForm">
        <div class="inputcontainer">
          <input type="text" :placeholder="currentUser.username" v-model="newUsername" autocomplete="username" name="username"/>
        </div>
        <ul class="checklist">
          <li :class="{ valid: usernameValid }">Felhasználónév: Min. 3 karakter, csak betű/szám/aláhúzás</li>
        </ul>
        <div class="inputcontainer">
          <input :type="passType" placeholder="Jelszó" v-model="newPassword" autocomplete="new-password" name="password"/>
