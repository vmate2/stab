<template>
  <div class="profileContainer">
    ...
    <client-only>
      <NuxtLink v-if="profile.uuid || profile.uuid !== ''" :to="`/stab/user/${profile.uuid}/profile`" class="link">
    ...
    .channel('realtime:stabcash')
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'stabcash',
          filter: 'id=eq.1',  // Correct filter format
        },
        (payload) => {
          balance.value = payload.new.balance;
        }
      )
      .subscribe();
